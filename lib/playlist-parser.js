...
export function parseHlsMaster(text, baseUrl = '') {
  ...
  for (const line of lines) {
    ...
    const info = attributes(line.slice(line.indexOf(':') + 1));
    if (info.TYPE === 'AUDIO' && info['GROUP-ID'] && info.URI) audioGroups.set(info['GROUP-ID'], { url: resolveUrl(info.URI, baseUrl), name: info.NAME || info.LANGUAGE || 'Audio' });
  }
  for (let index = 0; index < lines.length; index += 1) {
    ...
    variants.push({
      ...
      mime: 'application/vnd.apple.mpegurl', segments: [], audioGroup: info.AUDIO || '', audioPlaylistUrl: audioGroups.get(info.AUDIO)?.url || null,
      audioLabel: audioGroups.get(info.AUDIO)?.name || ''
    });
  }
...
export function isEncryptedHls(text) {
